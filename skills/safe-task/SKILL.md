...
- Uncommitted changes: !`git status --short 2>/dev/null | python3 -c "import sys; print(len(sys.stdin.read().splitlines()))"` files dirty
...
- Create a restore point: !`git stash create 2>/dev/null | python3 -c "import sys; sha=sys.stdin.read().strip(); print(sha + ' (stash object created — recover with \'git stash apply ' + sha + '\')') if sha else print('working tree clean — HEAD is your restore point')"  || echo "working tree clean — HEAD is your restore point"`
...
2. Danger-zone scan — does the task touch any of these?
   !`git ls-files 2>/dev/null | python3 -c "import sys,re; [print(f) for f in sys.stdin.read().split() if re.search(r'auth|login|password|secret|token|payment|billing|migration|schema|\.env|Dockerfile|deploy|prod',f,re.I)]" | head -15 || echo "(no danger-zone files found)"`
