...
**A single [Claude Code](https://claude.com/claude-code) plugin: 20 engineering-discipline skills + automatic hooks that turn your AI agent from a code generator into a disciplined engineering teammate.**
...
Everything — all 20 skills **and** the hooks — installs together. Type `/` to see them; the hooks start firing automatically. (Testing locally first? `/plugin marketplace add /path/to/claude-power-skills`.)
...
| Command | What it does |
| --- | --- |
| `/token-context` | Sets up the four-file context spine (`context.md`, `decisions.md`, `spec.md`, `plan.md`) with the prompt-cache breakpoint — run once per project to unlock 90% savings on stable context. |
| `/token-dispatch` | Dispatches sub-agents with a ≤300-token brief extracted from the spine files — prevents context bleed and keeps parallel agents cheap. |
| `/token-audit` | Audits the current session across 6 waste dimensions (cache hit rate, context growth, re-implementations, brief sizes, decisions captured, effort consistency) and outputs a ranked fix report. |
...
One install gets all 20 skills **and** the hooks. They're available globally, in every project. Edit a skill's `SKILL.md` and it takes effect immediately; after editing `hooks/` or `scripts/`, run `/reload-plugins`.
